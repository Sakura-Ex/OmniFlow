...
import type { TargetNodeData, TargetNodeMode } from '@/common/types/recipe'
...
export function TargetNode({ id, data }: NodeProps<TargetNodeData>) {
  ...
  return (
    <article className={`${styles['target-node']} ${styles[`target-node--${mode}`]}`}>
      <header className={styles['target-node__header']}>
        <p className={styles['target-node__kicker']}>OUTPUT DEMAND</p>
        <div className={styles['target-node__header-actions']}>
          <button
            className={`${styles['target-node__edit-btn']} nodrag`}
            onClick={() => onEdit(id, 'target', data)}
            title="设置"
          >⚙</button>
          <div className={styles['target-node__seg']}>
      ...
      <section className={shared['recipe-node__ports']}>
        <ul className={shared['recipe-node__port-list']}>
          {ports.map((port, index) => {
            ...
            return (
              <li className={`${shared['recipe-node__port']} ${shared['recipe-node__port--left']}${isGlobal ? ` ${shared['recipe-node__port--global']}` : ''}`} key={port._uid ?? `${id}-port-${index}`}>
                <div className={`${shared['recipe-node__port-core']} ${shared['recipe-node__port-core--left']}`}>
                  {isGlobal ? (
                    <svg
                  ...
                  <span className={`${shared['recipe-node__port-name']} ${shared['recipe-node__port-name--left']}`}>{port.id || '(未命名)'}</span>
