...
**ComputeFlow** 是一个专为硬核自动化游戏（如《我的世界》格雷科技、异星工厂等）设计的可视化产线矩阵求解器。

传统的产线计算器往往缺乏直观的拓扑连线，而纯节点编辑器又难以处理复杂的多入多出与环形死结（如水-氢气-燃烧循环）。ComputeFlow 将**现代化的节点交互 UI**与**底层的运筹学矩阵求解**完美融合，提供所见即所得的硬核解算体验。
...
services:
  api:
    build: .
    container_name: computeflow-api
    restart: always
    ports:
      - "8000:8000"
    environment:
